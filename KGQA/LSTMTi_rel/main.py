...
def validate(data_path, test_que_rel_path, device, model, word2idx, entity2idx, idx2rel, rel2idx, model_name):
    model.eval()
    data = process_text_file(data_path)
    que_rels = process_relQue_file(test_que_rel_path)
    sorce_data = get_value_tfidf(data)
    answers = []
    data_gen = data_generator(data=data, que_rels=que_rels, word2ix=word2idx, entity2idx=entity2idx, rel2idx=rel2idx)
    total_correct = 0
    error_count = 0
    for i in tqdm(range(len(data))):
        try:
            ...
            q_value_tfidf = d[4].to(device)
            rels_id = d[5].to(device)
            tail_test = torch.tensor(ans, dtype=torch.long).to(device)
            top_2 = model.get_score_ranked(head=head, sentence=question, sent_len=ques_len, q_value_tfidf=q_value_tfidf, relation=rels_id)
...
def process_relQue_file(relQue_file):
    que_rel = {}
    rel_file = open(relQue_file, 'r')
...
def train(data_path, que_rel_path, entity_path, relation_path, entity_dict, relation_dict,
          ...
          w_matrix, bn_list, valid_data_path=None, valid_que_rel_path=None):
    ...
    dataset = DatasetMetaQA(data=data, word2ix=word2ix, relations=r, que_rel=que_rel, rel2idx=relation2idx, entities=e, entity2idx=entity2idx, hops=hops)     #dataset = {data,entities,entity2idx,index_array,neg_dict,pos_dict,relations,word_to_ix}
    data_loader = DataLoaderMetaQA(dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers)   #shuffle = 是否打乱数据 num_workers = 多少个子线程用来加载数据，默认0只在主线程加载
    model = RelationExtractor(embedding_dim=embedding_dim, hidden_dim=hidden_dim, vocab_size=len(word2ix),
                              num_entities = len(idx2entity), relation_dim=relation_dim, pretrained_embeddings=embedding_matrix, rel_embedding=embedding_matrix_rel,
        ...
        for phase in phases:
            if phase == 'train':
                ...
                for i_batch, a in enumerate(loader):        #a = {[问题index,128], [问题长度,128], [主题词index, 128], [答案onehot, 128]}
                    ...
                    rels_id = a[5].to(device)

                    loss = model(sentence=question, p_head=positive_head, p_tail=positive_tail, question_len=sent_len, q_value_tfidf=q_value_tfidf, relation=rels_id)    #训练loss
                    loss.backward()
                    optimizer.step()
                ...
                model.eval()
                eps = 0.0001
                answers, score = validate(model=model, data_path= valid_data_path, test_que_rel_path=valid_que_rel_path, word2idx= word2ix, entity2idx= entity2idx, rel2idx=relation2idx, idx2rel=idx2relation, device=device, model_name=model_name)
                if score > best_score + eps:
                    best_score = score
                    no_update = 0
                    best_model = model.state_dict()
                    print(hops + " hop Validation accuracy increased from previous epoch", score)
                    _, test_score = validate(model=model, data_path= test_data_path, test_que_rel_path=test_que_rel_path, word2idx= word2ix, entity2idx= entity2idx, rel2idx=relation2idx, idx2rel=idx2relation, device=device, model_name=model_name)
...
def data_generator(data, que_rels, word2ix, entity2idx, rel2idx):
    for i in range(len(data)):
        data_sample = data[i]
        question_text = data_sample[1]
        head_name = data_sample[0]
        que = question_text.replace('NE', head_name)
        head = entity2idx[head_name.strip()]
        question = question_text.strip().split(' ')
        encoded_question = [word2ix[word.strip()] for word in question]
        q_value_tfidf = data_sample[3]
        relation = que_rels[que]

        ti_more = torch.full((1,400), q_value_tfidf[0],dtype=torch.float32)
        for i in range(len(q_value_tfidf)-1):
            ti_one = torch.full((1,400),q_value_tfidf[i+1],dtype=torch.float32)
            ti_more = torch.cat((ti_more,ti_one),dim=0)
        # if len(q_value_tfidf) != longest_sample:
        #     for j in range(longest_sample-len(q_value_tfidf)):
        ...
        for r in relation:
            rel = rel2idx[r]
            rel_id.append(rel)
        if hops == '1hop' or hops == '2hop':
            if len(rel_id) < 2:
                rel_id.append(rel2idx['pad'])
        elif hops == '3hop':
        ...
        yield torch.tensor(head, dtype=torch.long), torch.tensor(encoded_question, dtype=torch.long), ans, torch.tensor(len(encoded_question), dtype=torch.long), q_value_tfidf, torch.tensor(rel_id, dtype=torch.long), data_sample[1]
...
if args.kg_type == 'half':   #KG是否half
    ...
    que_rel_path = '../../data/MetaQA_relations/queRels_qa_train_' + hops + '_half.txt'
else:
    data_path = '../../data/QA_data/MetaQA/qa_train_' + hops + '.txt'
    que_rel_path = '../../data/MetaQA_relations/queRels_qa_train_' + hops + '.txt'
...
valid_que_rel_path = '../../data/MetaQA_relations/queRels_qa_dev_' + hops_without_old + '.txt'
test_que_rel_path = '../../data/MetaQA_relations/queRels_qa_test_' + hops_without_old + '.txt'
...
if args.mode == 'train':
    ...
    valid_que_rel_path=valid_que_rel_path,
