...
class RelationExtractor(nn.Module):
    ...
    def __init__(self, embedding_dim, relation_dim, num_entities, pretrained_embeddings, rel_embedding, device, entdrop, reldrop,
        ...
        self.halfRelDim2Tranf = nn.Linear(self.relation_dim, 512)
        self.Transf2Rel = nn.Linear(2560, self.mid2)
    ...
    def forward(self, question_tokenized, attention_mask, p_head, p_tail, words_ti_value, relation, relation_mask):
        question_embedding, words_embedding, src_mask = self.getQuestionEmbedding(question_tokenized, attention_mask)
        trg_mask = relation_mask
        # rel = relation.transpose(0,1)
        for b in range(relation.size(0)):
            rel = relation[b]
            for r in range(5):
                if r == 0:
                    rels = self.halfRelDim2Tranf(self.embedding_rel(rel[r])).unsqueeze(0)
                else:
                    temp2 = self.halfRelDim2Tranf(self.embedding_rel(rel[r])).unsqueeze(0)
                    rels = torch.cat((rels,temp2), dim=0)
            temp3 = rels.unsqueeze(0)
            if b == 0:
                rels_result = temp3
            else:
                rels_result = torch.cat((rels_result,temp3), dim=0)
        trg_rel_emb = rels_result
        ...
        words_TiBased_embedding = self.Transformer(words_embedding, src_mask, words_ti_value, trg_rel_emb, trg_mask_data=trg_mask)
        # words_TiBased_embedding *= words_ti_value
        # temp = torch.sum(words_TiBased_embedding, dim=1)
        for batch in range(words_TiBased_embedding.size(0)):
            rel_emb = words_TiBased_embedding[batch]
            rel_num = trg_mask[batch]
            for i in range(5):
                if rel_num[i] == 1 and i == 0:
                    real_emb = rel_emb[i].unsqueeze(0)
                elif rel_num[i] == 1 and i != 0:
                    real_emb = torch.cat((real_emb, rel_emb[i].unsqueeze(0)), dim=1)
                else:
                    rel_one = torch.full((1, 512), 0, dtype=torch.float32, device=self.device)
                    real_emb = torch.cat((real_emb, rel_one), dim=1)
            temp22 = real_emb
            if batch == 0:
                batch_real_emb = temp22
            else:
                batch_real_emb = torch.cat((batch_real_emb, temp22), dim=0)

        temp = self.Transf2Rel(batch_real_emb)
        ...
        # words_ti_value = self.words_ti_value_haddle(words_ti_value, question_tokenized)
        # words_embedding = self.bert2tranf(words_embedding)
        # words_tip_embedding = words_ti_value * words_embedding
        # temp = torch.sum(words_tip_embedding, dim=1)
    ...
    def get_score_ranked(self, head, question_tokenized, attention_mask, words_ti_value, relation, relation_mask):
        ...
        rel_emb = words_TiBased_embedding[0]
        for i in range(5):
            if trg_mask[i] == 1 and i == 0:
                real_emb = rel_emb[i].unsqueeze(0)
            elif trg_mask[i] == 1 and i != 0:
                real_emb = torch.cat((real_emb, rel_emb[i].unsqueeze(0)), dim=1)
            else:
                rel_one = torch.full((1, 512), 0, dtype=torch.float32, device=self.device)
                real_emb = torch.cat((real_emb, rel_one), dim=1)
