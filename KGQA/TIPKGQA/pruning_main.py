...
def validate_v2(model, device, train_dataset, rel2idx, idx2rel):
    model.eval()
    data = process_data_file('../../data/fbwq_full/pruning_cwq_dev.txt', rel2idx, idx2rel)
    num_correct = 0
    count = 0
    for i in tqdm(range(len(data))):
        ...
        isCorrect = False
        for x in top2:
            if x in rel_id_list:        # 只要预测到一个关系就是true
                isCorrect = True
        if isCorrect:
            num_correct += 1
        # else:
...
def train(batch_size, shuffle, num_workers, nb_epochs, gpu, use_cuda, patience, validate_every, lr, decay, ls):
    ...
    for line in f:
        ...
        idx2rel[id] = rel
    f.close()
    data = process_data_file('../../data/fbwq_full/pruning_cwq_train.txt', rel2idx, idx2rel)
        ...
        for phase in phases:
            if phase == 'train':
                ...
                if score > best_score + eps:
                    ...
                    torch.save(model.state_dict(), "checkpoints/pruning/cwq_best_mar12_3.pt")
