...
parser.add_argument('--hops', type=str, default='webqsp_half')
...
def validate_v2(data_path, device, model, train_dataloader, entity2idx, model_name):
    ...
    for i in tqdm(range(len(data))):
        ...
        words_ti_value = d[4]
        rel_id = d[5]
        relation_mask = d[6].to(device)
        tail_test = torch.tensor(ans, dtype=torch.long).to(device)
        scores = model.get_score_ranked(head=head, question_tokenized=question_tokenized, attention_mask=attention_mask, words_ti_value= words_ti_value, relation=rel_id, relation_mask=relation_mask)[0]
        ...
        for phase in phases:
            if phase == 'train':
                ...
                for i_batch, a in enumerate(loader):
                    ...
                    relation_mask = a[6].to(device)
                    loss = model(question_tokenized=question_tokenized,attention_mask=attention_mask, p_head=positive_head, p_tail=positive_tail, words_ti_value= words_ti_value, relation=relation, relation_mask=relation_mask)
                    loss.backward()
                    optimizer.step()
    ...
    for i in range(len(data)):
        ...
        if len_relation < 5:
            for i in range(5-len_relation):
                rel_id.append(dataloader.rel2idx['pad'])
        ...
        yield torch.tensor(head, dtype=torch.long), question_tokenized, attention_mask, ans, words_ti_value, torch.tensor(rel_id, dtype=torch.long), torch.tensor(relation_mask, dtype=torch.long), data_sample[1]
