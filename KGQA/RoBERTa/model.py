...
class RelationExtractor(nn.Module):
    ...
    def forward(self, words_tokenized, attention_masks, p_head, p_tail, words_ti_value):
        #question_embedding = self.getQuestionEmbedding(question_tokenized, attention_mask)
        words_embedding, src_mask = self.getWordsEmbedding(words_tokenized,attention_masks)
    ...
    def get_score_ranked(self, head, words_tokenized, attention_masks, words_ti_value):
        #question_embedding = self.getQuestionEmbedding(question_tokenized.unsqueeze(0), attention_mask.unsqueeze(0))
        words_embedding, src_mask = self.getWordsEmbedding(words_tokenized.unsqueeze(0),attention_masks.unsqueeze(0))
        words_embedding = self.bert2tranf(words_embedding)
        words_TiBased_embedding = self.Transformer(words_embedding, src_mask, words_ti_value)
        words_TiBased_embedding *= words_ti_value
        temp = torch.sum(words_TiBased_embedding, dim=1)
