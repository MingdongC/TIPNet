...
from dataloader import DatasetMetaQA, DataLoaderMetaQA, _collate_fn
...
def validate_v2(data_path, device, model, train_dataloader, entity2idx, model_name):
    ...
    for i in tqdm(range(len(data))):
        ...
        d = next(data_gen)
        head = d[0].to(device)
        words_tokenized = d[1].to(device)
        attention_masks = d[2].to(device)
        ans = d[3]
        words_ti_value = d[4]
        tail_test = torch.tensor(ans, dtype=torch.long).to(device)
        scores = model.get_score_ranked(head=head, words_tokenized=words_tokenized, attention_masks=attention_masks, words_ti_value= words_ti_value)[0]
...
def train(data_path, neg_batch_size, batch_size, shuffle, num_workers, nb_epochs, embedding_dim, hidden_dim, relation_dim, gpu, use_cuda,patience, freeze, validate_every, hops, lr, entdrop, reldrop, scoredrop, l3_reg, model_name, decay, ls, load_from, outfile, do_batch_norm, valid_data_path=None):
    ...
    data_loader = DataLoader(dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers,collate_fn=_collate_fn)
        ...
        for phase in phases:
            if phase == 'train':
                ...
                for i_batch, a in enumerate(loader):
                    model.zero_grad()
                    words_tokenized = a[0].to(device)
                    attention_masks = a[1].to(device)
                    positive_head = a[2].to(device)
                    positive_tail = a[3].to(device)
                    words_ti_value = a[4].to(device)
                    loss = model(words_tokenized=words_tokenized,attention_masks=attention_masks, p_head=positive_head, p_tail=positive_tail, words_ti_value= words_ti_value)
                    loss.backward()
                    optimizer.step()
...
def data_generator(data, dataloader, entity2idx):
    for i in range(len(data)):
        ...
        words_tokenized, attention_masks = dataloader.tokenize_words(question)
        ...
        yield torch.tensor(head, dtype=torch.long), words_tokenized, attention_masks, ans, words_ti_value, data_sample[1]
