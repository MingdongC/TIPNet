...
def validate_v2(data_path, device, model, train_dataloader, entity2idx, model_name):
    ...
    for i in tqdm(range(len(data))):
        ...
        scores = model.get_score_ranked(head=head, question_tokenized=question_tokenized, attention_mask=attention_mask, words_ti_value= words_ti_value)[0]
...
def data_generator(data, dataloader, entity2idx):
    for i in range(len(data)):
        ...
        yield torch.tensor(head, dtype=torch.long), question_tokenized, attention_mask, ans, words_ti_value, data_sample[1]
