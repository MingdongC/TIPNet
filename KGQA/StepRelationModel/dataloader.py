...
class DatasetMetaQA(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return question_tokenized, attention_mask, head_id, tail_onehot, #torch.tensor(words_ti_values, dtype=torch.float32)
    ...
    def tokenize_question(self, question):
        ...
        #question_tokenized = self.tokenizer.tokenize(que)
        #question_tokenized = self.pad_sequence(question_tokenized, 64)
        #question_tokenized = torch.tensor(self.tokenizer.encode(question_tokenized, add_special_tokens=False))
        #a = self.tokenizer.convert_ids_to_tokens(question_tokenized)
        attention_mask = []
...
def _collate_fn(batch):
    ...
    words = torch.zeros((minibatch_size, longest_sample, batch[0][0].size()[1]), dtype=torch.long)
    attentions = torch.zeros((minibatch_size,longest_sample, batch[0][0].size()[1]), dtype=torch.long)
    heads = []
