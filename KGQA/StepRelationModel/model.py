...
import random
from KGQA.StepRelationModel.Module import StepRelation


class RelationExtractor(nn.Module):

    def __init__(self, embedding_dim, batch_size, relation_dim, num_entities, pretrained_embeddings, device, entdrop, reldrop, scoredrop, l3_reg, model, ls, do_batch_norm, freeze=True):
        ...
        self.step_reason_model = StepRelation(device=self.device, relation_dim=self.hidden_dim, linear_drop=0.1, max_hop_num=3, ques_dim=self.hidden_dim)
        self.step_reason_model_valid = StepRelation(device=self.device, relation_dim=self.hidden_dim, linear_drop=0.1, max_hop_num=3, ques_dim=self.hidden_dim)
    ...
    def kge_loss(self, scores, targets):
        ...
        return self._klloss(
            scores, F.normalize(targets.float(), p=1, dim=1)
        )
    ...
    def ComplEx(self, head, relation):
        ...
        inter_entity_idx = torch.argmax(score, dim=-1, keepdim=True)
        for i in range(inter_entity_idx.size(0)):
            id = inter_entity_idx[i]
            emb = self.embedding.weight[id]
            if i==0:
                emb_cat = emb
            else:
                emb_cat = torch.cat((emb_cat,emb), dim=0)
    ...
    def forward(self, question_tokenized, attention_mask, p_head, p_tail):
        #q=(batch_size, embedding_dim)   w=(batch_size, numb_words, embedding_dim)
        question_embedding, words_embedding, words_mask = self.getQuestionEmbedding(question_tokenized, attention_mask)
        relation_per_hop, attention_list = self.step_reason_model(question_embedding, words_embedding, words_mask, question_embedding.size(0))
        hop_pred_linear = self.step_reason_model.predict_question_hops(question_embedding)
        hop_pred_linear= hop_pred_linear.unsqueeze(2)
        pred_entity = self.step_reason(relation_per_hop, hop_pred_linear, p_head)
        actual = p_tail
        if self.label_smoothing:
            actual = ((1.0-self.label_smoothing)*actual) + (1.0/actual.size(1)) 
        loss = self.loss(pred_entity, actual)
        if not self.freeze:
            if self.l3_reg:
    ...
    def step_reason(self, relation_per_hop, hop_pred_linear, p_head):

        entities = []
        head_entity = self.embedding(p_head)
        start_entity = head_entity
        for hop in range(self.max_hops):
            relation_cur_emb = relation_per_hop[hop]
            relation_cur_emb = self.applyNonLinear(relation_cur_emb)
            entity_scores, inter_entity = self.getScores(start_entity, relation_cur_emb)
            start_entity = inter_entity
            entities.append(inter_entity)
            entity_scores = entity_scores.unsqueeze(1)
            if hop == 0:
                entity_scores_all = entity_scores
            else:
                entity_scores_all = torch.cat((entity_scores_all, entity_scores), dim=1)
        entity_scores_all = F.log_softmax(entity_scores_all, dim=2)
        entity_scores_all = hop_pred_linear * entity_scores_all
        pred_entity_id = torch.argmax(torch.max(entity_scores_all,dim=2, keepdim=True)[0], dim=1)
        for bat in range(entity_scores_all.size(0)):
            idx = pred_entity_id[bat]
            temp = entity_scores_all[bat][idx]
            if bat == 0:
                pred_entity = temp
            else:
                pred_entity = torch.cat((pred_entity, temp), dim=0)
    ...
    def get_score_ranked(self, head, question_tokenized, attention_mask, words_ti_value):
        ...
        question_embedding, words_embedding, words_mask = self.getQuestionEmbedding(question_tokenized, attention_mask)
        relation_per_hop, attention_list = self.step_reason_model_valid(question_embedding, words_embedding, words_mask, question_embedding.size(0))
        hop_pred_linear = self.step_reason_model_valid.predict_question_hops(question_embedding)
        hop_pred_linear= hop_pred_linear.unsqueeze(2)
        head = head.unsqueeze(0)
        pred_entity = self.step_reason(relation_per_hop, hop_pred_linear, head)
