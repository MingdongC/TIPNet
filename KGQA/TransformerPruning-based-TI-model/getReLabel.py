...
def getAllRelations(head, tail):
    ...
    # =============================================================== #
    # paths_all = k_shortest_paths(G, head, tail, 500)
    # paths_real = []
    # if paths_all == []:
    #     return []
    # for path in paths_all:
    #     path_length = len(path)
    #     if hop + 1 == path_length:
    ...
    #         rels1 = triples_dict[(p[0], p[1])]
    #         if list(rels1) == []:
    #             rels1 = triples_dict[(p[1], p[0])]
    #         rels2 = triples_dict[(p[1], p[2])]
    #         if list(rels2) == []:
    #             rels2 = triples_dict[(p[2], p[1])]
    #         rels3 = triples_dict[p[2], p[3]]
    #         if list(rels3) == []:
    #             rels3 = triples_dict[(p[3], p[2])]
    #         relations = relations.union(rels1)
    #         relations = relations.union(rels2)
    #         relations = relations.union(rels3)
    #         return list(relations)
    #     else:
    #         print("hops not 1 2 3")
    #         return []
    # else:
    #     print("paths_real is none!")
    # ======================================================++++++++++ #
    ...
    if shortest_length == 1 or shortest_length == 0:
        # sl1+=1
        return list(triples_dict[(head, tail)])
    elif shortest_length == 2:
        # sl2 += 1
        paths = list(nx.shortest_path(G, head, tail))
        relations = set()
        for p in paths:
            rels1 = triples_dict[(p[0], p[1])]
            rels2 = triples_dict[(p[1], p[2])]
        ...
        # sl3 += 1
        paths = list(nx.shortest_simple_paths(G, head, tail))
        relations = set()
        for p in paths:
            rels1 = triples_dict[(p[0], p[1])]
            rels2 = triples_dict[(p[1], p[2])]
